...
void introSNE03(){
    ...
    cout << "----------------------" << endl;
    cout << "Model asocjacji" << endl;
}

vector<double> getClearVector_SNE03(double val){
    vector<double> tmp;
    for (int i = 0; i<25; i++) {
        tmp.push_back(val);
    }
...
vector<double> createZvector(vector<int> input) {
    ...
    output = getClearVector_SNE03(-1.0);
...
vector<vector<double> > createWvector(vector<double> w, vector<double> u) {
    ...
    getClearVector_SNE03(-1.0);

    for (int i=0; i<25; i++) {
        output.push_back(getClearVector_SNE03(-1.0));
        for (int j=0; j<25; j++) {
...
void printVector_SNE03(vector<double> vec) {
    for (int i=0; i<25; i++) {
        if (vec[i] == -1.0) {
            printf("[] ");
        } else {
